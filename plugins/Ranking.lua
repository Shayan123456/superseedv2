

local apikey = 
'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiIsImtpZCI6IjI4YTMxOGY3LTAwMDAtYTFlYi03ZmExLTJjNzQzM2M2Y2NhNSJ9..TTNEetGPs4X1_cEWRr1ARBjgz5sXYGjxgI0b4wirgYb2vr6S1m8fQBP8imt9rMGq7D8eafEDZVMn3QiBiSshAg' 
local function run(msg, matches)
 
 
 
 
 if matches[1]:lower() == 'memberrank' or matches[1]:lower() == 'clashmembers' or matches[1]:lower() == 'clanmembers' then
  local members = matches[2]
  members = string.upper(members)
  local curl = 'curl -X GET --header "Accept: application/json" --header "authorization: Bearer '..apikey..'" "https://api.clashofclans.com/v1/locations/'..members..'/rankings/players"'
  cmd = io.popen(curl)
  local result = cmd:read('*all')
  local jdat = json:decode(result)
  if jdat.reason then
      if jdat.reason == 'accessDenied' then return 'برای ثبت API Key خود به سایت زیر بروید\ndeveloper.clashofclans.com' end
   return '#Error\n'..jdat.reason
  end
  local leader = ""
  local coleader = ""
      local R3 = ""
  local R4 = ""
    local R5 = ""
  local items = jdat.items
  leader = 'Clan Moderators: \n'
   for i = 1, #items do
   if items[i].rank == 1 then
   leader = leader.."\n1 Player: "..items[i].name.."\nLevel: "..items[i].expLevel.."\nleague: "..items[i].trophies.."\nClan Tag: "..items[i].clan.tag.."\nClan Name: "..items[i].clan.name
   end
   if items[i].rank == 2 then
   coleader = coleader.."\n2 Player: "..items[i].name.."\nLevel: "..items[i].expLevel.."\nleague: "..items[i].trophies.."\nClan Tag: "..items[i].clan.tag.."\nClan Name: "..items[i].clan.name
  end
     if items[i].rank == 3 then
   R3 = R3.."\n3 Player: "..items[i].name.."\nLevel: "..items[i].expLevel.."\nleague: "..items[i].trophies.."\nClan Tag: "..items[i].clan.tag.."\nClan Name: "..items[i].clan.name
  end
     if items[i].rank == 4 then
   R4 = R4.."\n4 Player: "..items[i].name.."\nLevel: "..items[i].expLevel.."\nleague: "..items[i].trophies.."\nClan Tag: "..items[i].clan.tag.."\nClan Name: "..items[i].clan.name
  end
     if items[i].rank == 5 then
   R5 = R5.."\n5 Player: "..items[i].name.."\nLevel: "..items[i].expLevel.."\nleague: "..items[i].trophies.."\nClan Tag: "..items[i].clan.tag.."\nClan Name: "..items[i].clan.name
   end
  end
text = leader.."\n"..coleader.."\n"..R3.."\n"..R4.."\n"..R5.."\n"
  end
  text = text.."\n\n@shayan123hacker" -- اینو پاک کنی کل کد حرام میشه در ضمن ننه هرکی که اینو پاک یا عوض کنه :))
   cmd:close()
  return text
 end
end

return {
   patterns = {
"^[/!](clanrank) (.*)$",
   },
   run = run
}
